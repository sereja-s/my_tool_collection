...
// переносим title в начало блока lessonBlock:
...
// Клонирование(копирование) узлов: cloneNode

// Вызов elem.cloneNode(true) создаёт «глубокий» клон элемента – со всеми атрибутами и дочерними элементами. Если мы
// вызовем elem.cloneNode(false), тогда клон будет без дочерних элементов

// Получаем объект:
const textElem = document.querySelector('.lesson__text');

// клонируем его без дочерних элементов (без содержимого) по умолчанию:
// const cloneTextElem = textElem.cloneNode();

// или сделаем глубокое клонирование со всем содержимым:
const cloneTextElem = textElem.cloneNode(true);


// получим объект в который потом поместим клонированный(скопированный) объект
const block = document.querySelector('.lesson');
block.append(cloneTextElem);
...
// 1) Получение всех имён классов и их изменение является одним из наиболее часто используемых действий в скрипта Для этого было введено свойство "className": elem.className соответствует атрибуту "class".

// Получаем элемент с неким классом:
const elementClass = document.querySelector('.lesson__item-list-red');

// получим для него все имена классов:
const classNameForElement = elementClass.className;
console.log(classNameForElement);

// Перезаписываем имя класса:
// elementClass.className = 'new__class-red';

// Если мы присваиваем что-то elem.className, то это заменяет всю строку с классами. Иногда это то, что нам нужно, но
// часто мы хотим добавить/удалить один класс.



// 2) Для этого есть другое свойство: elem.classList.

// elem.classList – это специальный объект с методами для добавления/удаления одного класса
// Так что мы можем работать как со строкой полного класса, используя className, так и с отдельными классами, используя
// classList. Выбираем тот вариант, который нам удобнее.
...
const elementClli = document.querySelector('.lesson__item-list-red');

// Добавим класс:
elementClli.classList.add('active');
// Удалим класс:
elementClli.classList.remove('active');
// добавить класс, если его нет, иначе удалить
elementClli.classList.toggle('active');
// проверка наличия класса, возвращает true/false
elementClli.classList.contains('active');
console.log(elementClli.classList.contains('active')); // true

// Кроме того, classList является перебираемым, поэтому можно перечислить все классы при помощи for..of:
for (const className of elementClli.classList) {

	console.log(className); // lesson__item-list
	//  lesson__item-list-red
	//  active
}
...
// Свойство elem.style – это объект, который соответствует тому, что написано в атрибуте "style". Установка стиля elem.
// style.width="100px" работает так же, как наличие в атрибуте style строки width:100px.
...
// При этом каждое свойство пишется отдельно

// Задаём стиль с помощью CSS-свойства: 
elementClli.style.color = 'red';
elementClli.style.fontSize = '25px';
elementClli.style.fontWeight = '900';


// Получение значения свойства, которое записано в атрибуте: style (в html-коде):
console.log(elementClli.style.color);
...
// Затем мы можем удалить свойство style.display, чтобы вернуться к первоначальному состоянию. Вместо delete elem.style.
// display мы должны присвоить ему пустую строку: elem.style.display = "" (Если мы установим в style.display пустую
// строку, то браузер применит CSS-классы и встроенные стили, как если бы такого свойства style.display вообще не было)

//---------------------------------------------------------------------------------------------------------------------//

// Для задания нескольких стилей в одной строке используется специальное свойство style.cssText:

elementClli.style.cssText = `
margin-bottom: 30px;
background: white;
color: red;
font-size: 25px;
font-weight: 900;
`;
// Это свойство редко используется, потому что такое присваивание удаляет все существующие стили: оно не добавляет, а
// заменяет их. Можно ненароком удалить что-то нужное. Но его можно использовать, к примеру, для новых элементов, когда
// мы точно знаем, что не удалим существующий стиль.(То же самое можно сделать установкой атрибута: div.setAttribute('style', 'color: red...'))
...
// Для этого есть метод: getComputedStyle() Он применяется только для чтения (нельзя переназначить значение свойства)
...
const elGetComSty = getComputedStyle(elementClli);
// console.log(elGetComSty); // все стили объекта
console.log(elGetComSty.zIndex); // 100 (из css-файла)

// получим стили для псевдоэлемента:
const elStBrfore = getComputedStyle(elementClli, '::before');

console.log(elStBrfore.backgroundColor); // rgb(231, 231, 0)- цвет жёлтого кружочка

//===================================================================================================================//

// Атрибуты и свойства:

// Когда браузер загружает страницу, он «читает» (также говорят: «парсит») HTML и генерирует из него DOM-объекты. Для
// узлов-элементов большинство стандартных HTML-атрибутов автоматически становятся свойствами DOM-объектов.

// Например, для такого тега <body id="page"> у DOM-объекта будет такое свойство body.id="page".

// Но преобразование атрибута в свойство происходит не один-в-один! В этой главе мы уделим внимание различию этих двух
// понятий, чтобы посмотреть, как работать с ними, когда они одинаковые и когда разные


// DOM-свойства

// Ранее мы уже видели встроенные DOM-свойства. Их много. Но технически нас никто не ограничивает, и если этого мало –
// мы можем добавить своё собственное свойство.
...
// Получим список всех доступных свойств:
console.dir(elementClli); // список со всеми доступными свойствами
...
// Таким образом, когда у элемента есть id или другой стандартный атрибут, создаётся соответствующее свойство. Но этого
// не происходит, если атрибут нестандартный (Пожалуйста, учтите, что стандартный атрибут для одного тега может быть
// нестандартным для другого. Например, атрибут "type" является стандартным для элемента <input> (HTMLInputElement), но
// не является стандартным для <body> (HTMLBodyElement). Стандартные атрибуты описаны в спецификации для соответствующего класса элемента)
...
// Кроме этого, получить все атрибуты элемента можно с помощью свойства elem.attributes: коллекция объектов, которая
// принадлежит ко встроенному классу Attr со свойствами name и value
...
// При написании HTML мы используем много стандартных атрибутов. Но что насчёт нестандартных, пользовательских? Давайте
// посмотрим, полезны они или нет, и для чего они нужны.

// Иногда нестандартные атрибуты используются для передачи пользовательских данных из HTML в JavaScript, или чтобы
// «помечать» HTML-элементы для JavaScript Также они могут быть использованы, чтобы стилизовать элементы
...
// Все атрибуты, начинающиеся с префикса «data-», зарезервированы для использования программистами. Они доступны в свойстве dataset. Например, если у elem есть атрибут "data-about", то обратиться к нему можно как elem.dataset.about
...
// В большинстве ситуаций предпочтительнее использовать DOM-свойства. Нужно использовать атрибуты только тогда, когда
// DOM-свойства не подходят, когда нужны именно атрибуты, например:
...
// свойство href – всегда полный URL, а нам может понадобиться получить «оригинальное» значени